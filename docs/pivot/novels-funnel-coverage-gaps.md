...
| Purchase/unlock | Partially covered   | Query, paywall, recharge, payment-success, reader unlock-list, payments webhook, payments/coins/unlocks worker contracts, and payments/coins/unlocks service specs are listed, including `/coins` route-contract gap-fill from PR #547 and reader unlock-list coverage from PR #638. | Wave 2AY-A audited the repository-local web purchase/unlock seams in `docs/pivot/wave-2ay-closure.md` and found no narrowly missing assertion in the allowed scope. Remaining risk is live Stripe, Meta, production/staging, credentials, webhooks in real environments, and external purchase telemetry, all outside this document and #640. | Audited; no A2 test gap named        |
| Library         | Partially covered   | Resume-reading card, history e2e, auth worker-route characterization, reading-progress worker contracts/validation, and reading-progress service specs are listed.                                                                                                                   | Recovery surfaces have repository coverage, but entitlement-reflection combinations across subscription, individual unlock, progress, and unauthenticated states may deserve narrower future characterization.                                                                                                                                | P2                                   |
...
### P1: Purchase/unlock seam inventory and duplicate-coverage audit

Candidate scope: Wave 2AY-A audited the repository-local web purchase/unlock handoff among checkout creation, paywall checkout, recharge balance/checkout, post-payment return, and reader unlock-list surfaces using existing mock-only specs and docs. The audit table lives in `docs/pivot/wave-2ay-closure.md`.

Why: the purchase/unlock stage is the canonical monetization seam, but Requirements v2 narrowed this candidate to docs-first evidence before any new tests. The audit found existing assertions already cover the allowed repository-local web seams, so no A2 micro-characterization was named.

Out of scope for this candidate remains unchanged: live or test-mode Stripe calls, Meta Pixel/CAPI calls, webhook delivery from external systems, real credentials, schema changes, seed/import/data changes, R2 changes, `apps/api` Jest edits, and #233 / Kanban `t_030c3f29` verification.

### P2: Library entitlement reflection matrix

Candidate scope: characterize how account/library/continue-reading surfaces present active subscription access, individual chapter unlocks, expired/no-entitlement states, and empty progress using existing mocks.

Why: the library stage is covered, but its value depends on correctly reflecting entitlement and progress combinations after purchase or return visits.
